...
import { Search, Loader2, Sparkles, AlertCircle, Building2, Bed } from "lucide-react";
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-5 custom-scroll">
        <div className="mb-6 mt-5 space-y-3">
          <div className="flex items-center gap-2 mb-1">
             ...
             <h2 className="text-[13px] font-semibold text-zinc-200 tracking-tight">住宿推荐</h2>
          </div>
          ...
          <div className="relative group">
            <input
              ...
              placeholder={`搜索${confirmedCity || ""}酒店/民宿...`}
              className="w-full bg-zinc-900/60 border border-white/[0.06] rounded-xl pl-10 pr-4 py-3 text-[13px] text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-blue-500/40 focus:bg-zinc-900/80 transition-all duration-200"
            />
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-zinc-500 group-focus-within:text-blue-500 transition-colors" />
          </div>
          
          <button
            onClick={handleSearch}
            disabled={isSearching || !searchQuery.trim()}
            className="w-full relative overflow-hidden bg-blue-600 hover:bg-blue-500 disabled:bg-zinc-800/60 disabled:text-zinc-600 text-white text-[13px] font-medium py-2.5 rounded-xl transition-all duration-200 shadow-lg shadow-blue-900/20 active:scale-[0.98] disabled:active:scale-100 disabled:shadow-none group"
          >
        ...
        {hasSearched && !isSearching && searchResults.length === 0 && !searchError && (
          <div className="text-center py-10 animate-fade-in">
            <div className="w-14 h-14 bg-zinc-900/60 rounded-xl flex items-center justify-center mx-auto mb-3 border border-white/[0.04]">
              <Bed className="w-5 h-5 text-zinc-600" />
            </div>
            <p className="text-[13px] font-medium text-zinc-400">未找到相关住宿</p>
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="grid gap-3">
              {searchResults.map((result) => (
                <AnalysisCard key={result.note_id} result={result} type="hotel" />
              ))}
            </div>
          </div>
        )}
        ...
        {!hasSearched && (
          <div className="text-center py-12 opacity-60">
            <Bed className="w-10 h-10 text-zinc-700 mx-auto mb-3" />
            <p className="text-[13px] font-medium text-zinc-500">搜索当地住宿</p>
