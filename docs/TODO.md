...
- [ ] bug：右键节点弹窗距离右键点击位置遥远
- [ ] bug：无法搜索到其他城市，考虑为rules中强制使用mock数据的原因
- [ ] bug：同样，在搜索景点的时候也只能搜索到一个固定位置的地点，考虑为rules中强制使用mock数据的原因
- [ ] bug：右键收藏的位置以经纬度形式保存在收藏夹中，考虑修改为保存到附近的景点/标志地点
- [ ] new：行程日选择（Day1 2 3）只能通过shift+鼠标滚轮滑动，考虑修改为更人性化的滑动方法
- [ ] new：行程节点变多或者展开时到左侧栏中无法容纳时，容器右侧会出现滚动条，容器比例之间突然出现小的变动，考虑一直不显示滚动条
- [ ] new：点击节点的...的时候，在三点的位置呼出像右键点击节点时的弹窗
- [ ] feature：不只是景点，住宿和美食都需要有各自独立的收藏夹
